...
@objc public protocol SwiftySideMenuDelegate {
...
@objc public protocol SwiftySideMenuProtocol {
    var sideMenu : SwiftySideMenu? { get }
    func setContentViewController(_ contentViewController: UIViewController)
}

public enum SwiftySideMenuAnimation : Int {
    case none
    case `default`
}
/**
...
*/
public enum SwiftySideMenuPosition : Int {
    case left
    case right
}
...
public extension UIViewController {
    ...
    */
    ...
    func fixSideMenuSize() {
        if let navController = self.navigationController as? SwiftySideMenuNavigationController {
    ...
    */
    public func sideMenuController () -> SwiftySideMenuProtocol? {
        var iteration : UIViewController? = self.parent
        if (iteration == nil) {
            return topMostController()
        }
        repeat {
            if (iteration is SwiftySideMenuProtocol) {
                return iteration as? SwiftySideMenuProtocol
    ...
    internal func topMostController () -> SwiftySideMenuProtocol? {
        var topController : UIViewController? = UIApplication.shared.keyWindow?.rootViewController
        if (topController is UITabBarController) {
            topController = (topController as! UITabBarController).selectedViewController
        }
        var lastMenuProtocol : SwiftySideMenuProtocol?
        while (topController?.presentedViewController != nil) {
            if(topController?.presentedViewController is SwiftySideMenuProtocol) {
                lastMenuProtocol = topController?.presentedViewController as? SwiftySideMenuProtocol
            }
        ...
        if (lastMenuProtocol != nil) {
            ...
            return topController as? SwiftySideMenuProtocol
        }
    }
}

open class SwiftySideMenu : NSObject, UIGestureRecognizerDelegate {
    ...
    fileprivate var menuPosition:SwiftySideMenuPosition = .left
    ...
    open weak var delegate : SwiftySideMenuDelegate?
    ...
    */
    public init(sourceView: UIView, menuPosition: SwiftySideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
        super.init()
        ...
        self.panRecognizer = UIPanGestureRecognizer(target: self, action: #selector(SwiftySideMenu.handlePan(_:)))
        panRecognizer!.delegate = self
        sourceView.addGestureRecognizer(panRecognizer!)

        // Add right swipe gesture recognizer
        let rightSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(SwiftySideMenu.handleGesture(_:)))
        rightSwipeGestureRecognizer.delegate = self
        rightSwipeGestureRecognizer.direction =  UISwipeGestureRecognizerDirection.right

        // Add left swipe gesture recognizer
        let leftSwipeGestureRecognizer = UISwipeGestureRecognizer(target: self, action: #selector(SwiftySideMenu.handleGesture(_:)))
    ...
    */
    public convenience init(sourceView: UIView, menuViewController: UIViewController, menuPosition: SwiftySideMenuPosition, blurStyle: UIBlurEffectStyle = .light) {
...
extension SwiftySideMenu: UIDynamicAnimatorDelegate {
